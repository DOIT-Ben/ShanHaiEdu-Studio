...
状态：`Accepted / V1-1 done / V1-2 implementation checkpoint / closure pending`
...
| V1-2 | Tool与Agent Tool注册 | 已形成Registry、调用信封、Router和初版锚点门；当前先修独立Critic、结构化返修、premise/正例覆盖与默认授权测试 | 通过V1-2 checkpoint全部封板项并形成closeout后才算done |
...
第一项开发任务固定为V1-2正式封板：在合同、Router和注入Executor测试层补齐独立`delivery_critic.review`课程锚点硬门、保留结构化返修结果、扫描premise并增加不过度约束正例、覆盖默认数据库授权路径。完成专项测试、全量测试、生产构建、`git diff --check`和V1-2 closeout后，才进入V1-3；V1-2不得把Agent Tool标记为生产可执行。
