...
```text
local-real-v1-control-plane-assistant-ui-refactor-plan.md
local-real-v1-control-plane-assistant-ui-refactor-test-plan.md
local-real-v1-control-plane-assistant-ui-refactor-closeout.md
...
local-real-v1-v1-9r-agent-autonomy-human-gate-recovery-closeout.md
local-real-v1-v1-9-unique-real-product-e2e-plan.md
local-real-v1-v1-9-unique-real-product-e2e-test-plan.md
```

控制面与assistant-ui定向重构、R5真实桌面、TaskBrief预算、持久调用台账、自动续段checkpoint、Skill强制预检、媒体信任边界、课堂视觉图Tool和只观察型runner均已关闭。A23新运行的不可变manifest、`v1-9-run-state.v2`、多轮审核和同TurnJob恢复已实现，当前按V1-9专用plan/test-plan执行全量仓内回归与只读preflight；全部通过后才创建新run并消耗唯一一次真实Provider全链路。V1发布前真实浏览器门只运行桌面视口；既有窄屏合同和历史证据继续保留。
...
以下文档属于独立切片或 V1 发布后的规划，不得打断当前 V1 主线。`v1-1-feedback-closed-loop-*`中的assistant-ui通用反馈规划不再控制当前Runtime时序，当前assistant-ui实施以本页活动重构计划为准：
