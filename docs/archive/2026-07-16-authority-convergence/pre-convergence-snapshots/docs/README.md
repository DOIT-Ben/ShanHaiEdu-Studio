...
```text
docs\architecture\decisions\2026-07-14-adr-assistant-ui前移并统一控制面消息边界.md
...
docs\stages\local-real-v1-control-plane-assistant-ui-refactor-closeout.md
...
docs\stages\local-real-v1-v1-9r-agent-autonomy-human-gate-recovery-closeout.md
docs\stages\local-real-v1-v1-9-unique-real-product-e2e-plan.md
docs\stages\local-real-v1-v1-9-unique-real-product-e2e-test-plan.md
...
assistant-ui 对话 Runtime 已前移到当前 V1 控制面重构。以下文件名保留 `v1-1` 历史命名，但其中消息合同与技术边界供当前重构复用；实施时序和当前验收统一服从本页第 1 节列出的新 ADR、plan 与 test plan：
...
当前只前移 assistant-ui、MessagePart、事件恢复和必要运行状态；旧 ADR 只保留技术来源价值，不再控制版本时序或 390px 验收。反馈闭环、搜索及其他 V1.1 扩张仍只在 V1-10 发布门关闭后实施。
