...
本审计用于收口 `mainline/local-real-mvp` 在 M0-M40-B 后的真实状态，明确哪些能力已经通过本地验证、哪些只是 readiness、哪些仍不能宣称完成。
...
- 已具备完整登录 UI、OAuth/SSO、组织/班级权限或生产级多人协作。
...
- M40-A/M40-B 已有公网认证服务端模型、会话边界、membership 授权底座、CSRF-ready helper、审计脱敏 helper 和密码登录 API 最小闭环；但登录 UI、OAuth/SSO、完整 CSRF 落库校验、管理员/共享协作 UI、审计查询 UI、完整 CSP、HSTS、rate limit 和登录风控仍未完成。
...
- 当前已具备本地会话、项目 owner 隔离、公网认证服务端底座和密码登录 API 最小闭环，但这不是完整公网认证；登录 UI、OAuth/SSO、组织/班级、共享协作、管理员 UI 和审计查询仍未完成。
...
1. 做 M40-C 登录 UI 与 password auth mode 浏览器闭环，验证工作台在公网认证模式下不回退本地匿名 actor。
