...
function fixture() {
  ...
  roots.push(root);
  const roles = ["lesson_plan", "pptx", "pdf", "video"] as const;
  const files: FinalPackageFile[] = roles.map((role) => {
    ...
    return { role, filePath, packageFileName: role === "lesson_plan" ? "教案.md" : `${role}.${role}`, sha256: createHash("sha256").update(readFileSync(filePath)).digest("hex"), courseVersionId: "percentage-v1", courseAnchor: "百分数的意义", reviewBatchId: "review-20260712", deliveryStatus: "final_eligible" };
  });
  const classroomRunSpec: ClassroomRunSpec = { schemaVersion: "classroom-run-spec.v1", courseVersionId: "percentage-v1", courseAnchor: "百分数的意义", sequence: [
...
describe("V1 Stage 5 versioned final package", () => {
  it("builds and reverse-verifies a hash-bound package without claiming teacher signoff", async () => {
    ...
    expect(result.manifest).toMatchObject({ courseVersionId: "percentage-v1", packageStatus: "integration_review_passed", teacherSignoff: false, mediaEvidence: { pptx: { slideCount: 12 }, pdf: { pageCount: 12 } } });
