...
describe("runtime output quality", () => {
  it.each(tasks)("adds a teacher-review checklist for %s", async (task) => {
    const result = await new DeterministicRuntime().run(makeInput(task));
  ...
  it("includes required lesson-plan fields", async () => {
    const result = await new DeterministicRuntime().run(makeInput("lesson_plan"));
  ...
  it("includes required PPT outline fields", async () => {
    const result = await new DeterministicRuntime().run(makeInput("ppt_outline"));
  ...
  it("includes intro-video fields without pretending to make a finished video", async () => {
    const result = await new DeterministicRuntime().run(makeInput("intro_video_plan"));
  ...
  it("keeps final delivery honest about unfinished files", async () => {
    const result = await new DeterministicRuntime().run(makeInput("final_delivery_checklist"));
