...
状态：`Accepted / V1-1 to V1-6 done / V1-7 next`
...
第一项开发任务已经切换为V1-7视频内部编排：先冻结Concept Selection、课程锚点六硬门、成片后二次审查和镜头级返修合同，再接入产品Main Agent。V1-6已完成专项71/71、全量、生产构建和closeout；不得重新从PPT外部编排或真实交付包路线起步。

V1-7期间仍不得调用真实图片/视频Provider、制作新整包、移动既有标签，也不得由外部Codex替Main Agent选择创意、批准课程锚点或决定返修范围。
