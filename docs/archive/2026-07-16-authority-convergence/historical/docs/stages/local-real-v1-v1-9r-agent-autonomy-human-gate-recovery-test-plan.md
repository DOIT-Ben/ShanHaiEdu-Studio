...
状态：`Accepted / R0-R5 passed / V1-9 unique real Provider E2E next`
...
- PPT候选红绿测试必须证明：模型语义候选完整但回传错误TaskBrief digest、Artifact ID/digest时，服务端丢弃这些不可信字段并使用已验证ExecutionEnvelope、有效TaskBrief和当前可信`ppt_draft`投影；缺少Envelope/TaskBrief或可信`ppt_draft`分别以`ppt_design_task_binding_missing`、`ppt_design_evidence_binding_missing`失败关闭。
- PPT候选的缺失与语义无效分别返回`ppt_design_candidate_missing`、`ppt_design_candidate_semantics_invalid`；四类reasonCode必须出现在ValidationReport和Observation。candidate-only结果通过候选Validator，但真实媒体Adapter继续要求完整`PptDesignPackage`。
