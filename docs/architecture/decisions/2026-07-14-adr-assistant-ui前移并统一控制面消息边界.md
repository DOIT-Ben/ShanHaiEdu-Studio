...
- Main Agent 的 OpenAI-compatible Responses 调用使用单请求真流式语义事件；不得先同步等待终态再伪造逐字显示，也不得在流式失败后自动追加等价非流式请求。
- Provider 原始 JSON、function-call 参数 delta 和推理内容只在服务端累积。教师端只接收安全的 assistant 正文 delta、Tool 开始、Observation、失败位置和恢复状态。
- 静态 instructions、Tool 定义和结构合同保持稳定前缀，并使用稳定 `prompt_cache_key`；同一 ReAct 运行在 Provider 返回 response id 时使用 `previous_response_id` 续接，只发送新的 Tool Observation。
- 每次请求记录 cache read/write tokens、首协议事件、首可见文本、chunk 数和总耗时。流式投影失败不能改变 Provider 结果或业务控制流。
- 正式 assistant 消息只在结构化终态通过校验后持久化一次；流式事件是可重放投影，不得成为第二消息真源。
...
```text
docs\archive\2026-07-16-authority-convergence\historical\docs\stages\local-real-v1-control-plane-assistant-ui-refactor-plan.md
docs\archive\2026-07-16-authority-convergence\historical\docs\stages\local-real-v1-control-plane-assistant-ui-refactor-test-plan.md
```
