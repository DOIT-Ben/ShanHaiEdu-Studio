# 历史 ADR：原 V1.1 采用 assistant-ui 与 AG-UI 兼容事件层

日期：2026-07-14

状态：Superseded in timing by `2026-07-14-adr-assistant-ui前移并统一控制面消息边界.md`；仅保留不冲突的技术边界，当前版本时序、Tool责任和验收视口均以新 ADR 为准

## 1. 背景

当前对话层把消息正文、计划卡、快捷回复、成果卡和运行状态分散在项目自有组件与完整Snapshot轮询中。回复呈现、长任务活动流、Tool状态、HumanGate、成果引用、重试和刷新恢复在多份需求中重复声明，继续逐项自研会形成多套消息语义。

V1仍在修复Main Agent自主编排与HumanGate，不能用前端框架迁移代替控制层修复。2026-07-14用户明确要求将assistant-ui与控制层一起重构，因此本ADR原“V1后实施”时序已被新ADR替代。

## 2. 决策

以下技术路线被当前 V1 重构复用，但不再限定为 V1.1 才实施：
...
不采用。assistant-ui必须与项目自有MessagePart、事件真源和控制面责任边界同时迁移，不能掩盖或绕过服务端状态错误。
...
- 当前 V1 只执行 1366x768 桌面真实浏览器验收；390x844 保留既有合同和历史证据，新的窄屏真实验收在 V1 发布后单独安排。
- 功能开关切换及旧UI回退演练。
