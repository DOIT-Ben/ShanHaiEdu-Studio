> 历史口径提示：本文为 2026-07-07 当前状态审计，仅保留为历史证据。当前开发与验收以 `docs\product\current-requirements-baseline.md` 为唯一权威口径；本文中关于“文本大纲生成最小 PPTX”等阶段性过渡能力不得作为当前真实交付标准。
...
本审计用于收口 `mainline/local-real-mvp` 在 M0-M40-C 后的真实状态，明确哪些能力已经通过本地验证、哪些只是 readiness、哪些仍不能宣称完成。
...
- `docs\stages\local-real-mvp-m40b-password-auth-report.md`
- `docs\stages\local-real-mvp-m40c-password-auth-ui-report.md`
...
M40-B 已把 M40-A 的公网认证底座推进为服务端密码登录最小闭环：

- 新增 scrypt 密码 hash/verify helper，密码只保存带 salt 的 verifier。
- 新增注册、登录、退出和当前用户服务。
- 新增 `POST /api/auth/register`、`POST /api/auth/login`、`POST /api/auth/logout`、`GET /api/auth/me`。
- public session 明文 token 只进入 `shanhai_session` httpOnly cookie，数据库只保存 hash。
- 登录失败统一返回“账号或密码不正确。”，不泄露账户是否存在。
- auth API 响应体不返回 `passwordHash`、session 明文 token 或 session hash。
- 注册、登录、退出审计继续走 metadata 脱敏 helper。
...
- `npm run build` 通过，Next 已识别 4 个 `/api/auth/*` 动态 route。
- `npm run test:e2e:stage7` 通过：Chromium desktop 1 passed。
- standalone HTTP smoke 通过：临时 SQLite 下 register 201、me 200/authenticated、login 200、logout 200/revoked。

因此当前可以表述为“密码登录服务端 API、强哈希、session hash 入库、退出撤销、当前用户查询和通用登录失败响应已具备”，但仍不能表述为“登录 UI、OAuth/SSO、完整公网认证体系、rate limit、登录风控或公网部署已完成”。

### 3.39 M40-C 密码登录 UI 与浏览器闭环

M40-C 已把 M40-B 的密码登录 API 接入浏览器入口：

- password auth mode 下未登录先显示登录门禁。
- 支持浏览器创建账号、登录、刷新恢复和退出。
- 登录后才挂载工作台 controller，避免未登录时继续加载项目并回退匿名 actor。
- 顶栏显示当前教师并提供退出入口。
- 默认本地模式不强制登录，既有本地浏览器主链路保持可用。
- password auth UI 已在 Chromium desktop 和 Chromium narrow 下通过浏览器验收。
...
因此当前可以表述为“password auth mode 的浏览器登录门禁、创建账号、登录、刷新恢复、退出和默认本地模式回归已通过”，但仍不能表述为“OAuth/SSO、组织协作、完整登录风控、远端生产认证或公网部署已完成”。
...
- M40-A/M40-B/M40-C 已有公网认证服务端模型、会话边界、membership 授权底座、CSRF-ready helper、审计脱敏 helper、密码登录 API 最小闭环和浏览器登录门禁；但 OAuth/SSO、完整 CSRF 落库校验、管理员/共享协作 UI、审计查询 UI、完整 CSP、HSTS、rate limit 和登录风控仍未完成。
...
- 当前已具备本地会话、项目 owner 隔离、公网认证服务端底座、密码登录 API 最小闭环和浏览器登录门禁，但这不是完整公网认证；OAuth/SSO、组织/班级、共享协作、管理员 UI 和审计查询仍未完成。
...
1. 做 M40-D 账号权限系统继续完善，覆盖 CSRF 落库校验接入、登录频率保护、项目 membership 管理入口或最小邀请/共享模型。
