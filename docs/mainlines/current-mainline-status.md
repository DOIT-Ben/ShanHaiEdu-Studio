...
| M71A project lifecycle and feedback polish | requirements approved / planning pending | 用户本地验收新增：反馈选中态、轻量问候、项目重命名、归档、回收站和恢复；不含永久删除 |

## 3. 当前优先级

当前优先级从高到低：

1. 先完成用户明确要求的 M71A 本地验收修正：反馈选中态、自然问候、项目重命名、归档、回收站和恢复。
2. M71A 完成后恢复上线前生产门禁：真实外部 provider smoke 和一条真实教师任务端到端验收。
...
1. 强化反馈选中态与提交按钮层级。
2. 调整真实主对话 Agent 的轻量问候策略。
3. 实现项目重命名、归档、软删除、回收站和恢复的全栈闭环。
4. M71A 验收后回到目标服务器生产门禁和真实 provider smoke。
5. 生产门禁关闭后再评估真实用户邀请，并继续 M71 视频结构化前置链路。
