...
import type { ExternalAuditRepairHandoff, ExternalAuditTaskBinding } from "@/server/conversation/external-audit-repair-contract";
...
type RepairContext = Awaited<ReturnType<typeof loadRepairContext>>;
type RepairProjection = ReturnType<typeof buildRepairProjection>;

export async function commitExternalAuditRepairEvidence(input: {
  client: PrismaClient;
  binding: ExternalAuditRunStateBinding;
  handoff: ExternalAuditRepairHandoff;
}) {
  const { binding, handoff } = input;
  ...
  return input.client.$transaction(async (tx) => {
    ...
    if (existingObservation) {
      return replayExternalAuditRepair({ tx, binding, handoff, observationId, existingObservation });
    }

    const context = await loadRepairContext(tx, binding);
    const projection = buildRepairProjection({ binding, handoff, observationId, ...context });
    await persistRepair(tx, { binding, handoff, observationId, ...context }, projection);
    return result("committed", binding, handoff, observationId, projection.nextPlanRevision);
  });
}

async function replayExternalAuditRepair(input: {
  tx: TransactionClient;
  binding: ExternalAuditRunStateBinding;
  handoff: ExternalAuditRepairHandoff;
  observationId: string;
  existingObservation: { payloadJson: string; projectId: string; taskId: string; intentEpoch: number };
}) {
  const { binding, handoff, observationId, existingObservation } = input;
  ...
  const committedPlanRevision = await requireCommittedReplay(input.tx, binding, handoff, payload);
  return result("replayed", binding, handoff, observationId, committedPlanRevision);
}

async function loadRepairContext(tx: TransactionClient, binding: ExternalAuditRunStateBinding) {
  ...
  return { taskBrief, teacherMessage, turnJob, previousSnapshot, previousCheckpoint };
}

function buildRepairProjection(input: RepairContext & {
  binding: ExternalAuditRunStateBinding;
  handoff: ExternalAuditRepairHandoff;
  observationId: string;
}) {
  const { binding, handoff, observationId, taskBrief, previousSnapshot, previousCheckpoint } = input;
  ...
  const messageMetadata = appendRunCheckpointMetadata(
    appendAgentObservationMetadata(parseJson(input.teacherMessage.metadataJson, "external_audit_message_metadata_invalid"), observation),
    runCheckpoint,
  );
  ...
  return { nextPlanRevision, observation, checkpoint, messageMetadata, nextSnapshot };
}
...
async function persistRepair(
  ...
  const { binding, handoff, observationId, turnJob } = input;
  const { nextPlanRevision, observation, checkpoint, messageMetadata, nextSnapshot } = projection;
  ...
  await tx.taskAggregate.update({
    ...
    data: { planRevision: nextPlanRevision, status: "paused_recovery", checkpointJson: JSON.stringify(checkpoint) },
  });
...
async function requireCommittedReplay(
  ...
  const taskBrief = aggregate
    ...
    : null;
  if (!aggregate || aggregate.projectId !== binding.projectId || aggregate.taskId !== binding.taskId ||
      taskBrief?.digest !== binding.taskBriefDigest || aggregate.planRevision < committedPlanRevision || !event ||
      event.projectId !== binding.projectId || event.taskId !== binding.taskId || event.intentEpoch !== binding.intentEpoch ||
      !snapshot || snapshot.projectId !== binding.projectId || snapshot.taskId !== binding.taskId ||
      snapshot.intentEpoch !== binding.intentEpoch || snapshot.planRevision !== committedPlanRevision) {
