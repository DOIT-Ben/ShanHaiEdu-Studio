...
- 状态：`implementation evidence retained / product behavior reopened by RQ-038`
...
### RQ-039 V1.1 assistant-ui对话Runtime与AG-UI兼容事件层
...
- 目标：V1.1以assistant-ui作为教师对话区唯一UI Runtime，通过ExternalStoreRuntime复用现有服务端消息和权限状态；建立项目自有MessagePart与AG-UI兼容事件Adapter，统一承载文本、活动、计划、Tool状态、Artifact引用、质量摘要、HumanGate、下一步和错误恢复。
- 唯一边界：
  ...
  - BlockNote属于V1.3-V1.5文档成果编辑，不进入V1.1对话Runtime迁移。
- 实施：V1.1-0冻结合同与V1差异；V1.1-1完成assistant-ui Adapter、历史消息兼容和安全Renderer；V1.1-2完成AG-UI兼容流、断线恢复、活动流和activePlan；后续反馈、搜索和验收只消费统一Runtime。
- 迁移：加法字段、旧消息确定性映射、影子投影、测试账号功能开关、正式切换和旧UI回退；禁止双写业务状态，旧UI只在满足删除条件后另行清理。
- 验收：历史消息无损；真流式、重复/乱序/断线续接和快照校正通过；受控编辑/重试/分支不绕过IntentEpoch与ActionPolicy；两用户隔离；1366x768与390px无裸Markdown、注入、重叠、工程词或伪状态；V1业务路径和旧UI回退无回归。
- 权威需求：`docs\product\v1-1-assistant-ui-conversation-runtime-requirements.md`。
- 架构决策：`docs\architecture\decisions\2026-07-14-adr-v1-1采用assistant-ui与AG-UI兼容事件层.md`。
