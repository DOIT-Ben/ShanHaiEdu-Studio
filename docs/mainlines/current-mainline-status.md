...
当前唯一动作是按活动计划继续VR-A14/A16。R1已从runner child env移除`SHANHAI_RECOVER_RETRYABLE_TURNS_ON_START`，产品只在当前manifest/state路径同时存在时检查恢复；下一切片必须从精确SQLite身份派生typed recovery disposition。project、task、epoch、teacher message、TurnJob、checkpoint、actor/auth/session、plan任一缺失或错绑仍须在服务ready前失败关闭，不取项目全局latest。真实Provider连续3组仍等待用户另行批准。
