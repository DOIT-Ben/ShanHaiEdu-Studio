...
状态：`Accepted / V1-1 done / V1-2 production candidate / 126 of 134 focused tests passing`
...
| V1-2 | Tool与Agent Tool注册 | 已形成Registry、调用信封、Router、独立Critic、结构化返修与默认授权候选；审批状态红灯已关闭，当前收口Executor结果字段封闭、签名审查目标集合绑定和可执行返修合同 | 通过V1-2 checkpoint全部封板项并形成closeout后才算done |
...
第一项开发任务固定为V1-2现有生产候选的正式封板：保留已经转绿的审批状态一致性、前置Tool正向白名单、否定语义正例和结构化Schema约束，关闭当前8个红测，具体为Executor结果顶层权威字段封闭、所有Critic输入/输出/finding locator对签名review target的集合绑定，以及`rework_required/blocked`报告的finding和上游责任阶段完整性。完成专项测试、全量测试、生产构建、SQLite连续初始化、`git diff --check`和V1-2 closeout后，才进入V1-3；V1-2不得把Agent Tool标记为生产可执行。
