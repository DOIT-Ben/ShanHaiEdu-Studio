...
本审计用于收口 `mainline/local-real-mvp` 在 M0-M35 后的真实状态，明确哪些能力已经通过本地验证、哪些只是 readiness、哪些仍不能宣称完成。
...
- 已完成正式签名客户端安装包、自动更新、安装/卸载、窗口生命周期或系统权限验收。
...
- 生产就绪度：约 88%-90%。真实文本 smoke、Coze PPT `/run` smoke、Coze PPT artifact adapter、图片 live smoke、图片 artifact adapter、图片下载 route、图片材料包集成、视频 live smoke、视频 artifact adapter、视频下载 route、视频材料包集成、教师 UI 真实生成入口、浏览器联动验证、可配置部署卷准备、本地权限最小闭环、账号安全加固、真实生成任务状态基础、Next standalone 准备、生产预检、本地生产 SQLite 初始化、客户端 exe 验证准备、真实 Windows 未签名候选包生成和默认 unpacked exe smoke 已通过，但业务节点真实模型全面接入、Coze 官方 OpenAPI 主链路、独立 worker、公网正式认证、正式签名客户端安装包、安装/卸载验收、远端部署、安全与运维仍未完成。
...
- M35 已有默认安装包 smoke，证明候选包、打包资源安全和 unpacked exe HTTP 200；但显式静默安装/卸载 smoke 失败，原因是安装文件已解压但卸载器未生成。
...
1. 做 M36 安装器 blocker 排障：定位 NSIS assisted installer 静默安装不退出且不生成卸载器的根因。
2. 对比 one-click NSIS、assisted NSIS 和 portable target，选择本地 MVP 首个可验收安装器路线。
3. 修 Next standalone tracing 根因，减少桌面打包对安全过滤脚本的依赖。
