...
export function verifyModelGatewaySmokeReceipt({ root = process.cwd(), now = new Date(), maxAgeHours = 24 } = {}) {
  ...
  for (const capability of ["models", "agent", "text", "image", "tts", "video"]) {
  ...
  if (!["agent", "text", "image", "tts", "video"].every((capability) => checks[capability].requestIdPresent === true)) {
    throw new Error("MODEL_GATEWAY_REQUEST_ID_MISSING");
  }
  for (const capability of ["agent", "text", "image", "tts", "video"]) {
...
export function gatewayConfigDigest(configs) {
  ...
  for (const capability of ["agent", "text", "image", "video", "tts"]) {
