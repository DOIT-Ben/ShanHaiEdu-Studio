...
- M39 后候选客户端已启用 asar，并将 `desktop-bundle/**` 与 `node_modules/**` unpack 到真实目录，避免 Next standalone server 在 asar 虚拟目录下启动失败。
- M39 后桌面端会在 `userData` 下建立 `data`、`artifact-storage-root`、`logs` 和 `crash-dumps`。
...
- 正式签名客户端安装包、自动更新、窗口生命周期和系统权限验收；其中 M39 已证明未签名候选包具备基础 metadata、图标、asar/unpack、日志/崩溃目录和默认启动 smoke，但正式签名、自动更新和人工可见向导截图仍未完成。
