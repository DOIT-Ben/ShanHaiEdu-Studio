...
const taskTemplates: Record<AgentRuntimeTask, DraftTemplate> = {
  ...
  knowledge_anchor_extract: {
    title: "视频知识锚点",
    summary: "已从教案中提取导入视频必须回落的知识锚点、易错点和课堂问题。",
    sections: (input) => [
      section("知识锚点", [`核心知识：${input.projectContext.topic}。`, "只作为导入回落点，不在视频中提前讲完结论。"]),
      section("学生易错点", ["容易把生活现象和数学表达割裂。", "容易只看热闹，忽略需要进入课堂探究的问题。"]),
      section("生活关联点", ["使用学生熟悉的生活情境制造观察需求。", "画面只提出问题，不替代教材和教师讲解。"]),
      section("可创意化表达点", ["用冲突、悬疑或任务引出观察。", "结尾用问题把学生带回课堂。"]),
      section("课堂回落问题", [`这些现象为什么需要用${input.projectContext.topic}来解释？`]),
    ],
  },
  creative_theme_generate: {
    title: "导入创意主题",
    summary: "已围绕知识锚点生成可供确认的导入创意主题。",
    sections: (input) => [
      section("候选主题", ["主题 A：消失的折扣线索。", "主题 B：班级投票里的反常结果。"]),
      section("一句话故事", [`学生在熟悉场景中发现一个和${input.projectContext.topic}有关但暂时解释不了的现象。`]),
      section("创意类型", ["悬疑 / 任务。"]),
      section("绑定知识锚点", [`绑定${input.projectContext.topic}的生活表达和课堂回落问题。`]),
      section("课堂适配原因", ["只负责激发问题，不直接讲知识结论。"]),
