...
const taskTemplates: Record<AgentRuntimeTask, DraftTemplate> = {
  ...
  ppt_design: {
    ...
    sections: (input) => [
      ...
      section("第 1 页四层设计", [
        "底图：纯白背景，右侧放轻量生活场景插画，留出大面积讲解空间。",
        "元素：一个醒目的问题气泡、三枚数量变化小卡片、细线连接到课题。",
        `文字：主标题《${input.projectContext.topic}》，副标题“从生活问题开始观察”。`,
        "排版：标题左上 28-32px，问题气泡居中偏左，插画不超过页面宽度 35%。",
      ]),
      section("第 2 页四层设计", [
        "底图：白底加浅灰网格，模拟黑板上的观察区。",
        "元素：三组可比较数据卡、学生举手图标、教师追问箭头。",
        "文字：只保留一个核心问题“你发现了哪些变化？”和两条学生表达占位。",
        "排版：数据卡横向排列，追问箭头从左到右引导课堂讨论。",
      ]),
      section("第 3-12 页四层延展规则", [
        "底图：继续使用白底和浅色分区，概念页用板书区，练习页用题卡区。",
        "元素：每页最多 3 类核心元素，优先数量线、比例关系、板书框和学生回答框。",
        "文字：每页不超过 1 个主问题、2 个提示点、1 个课堂口令。",
        "排版：左侧问题，右侧材料；上方任务，下方学生操作；总结页用三段式收束。",
      ]),
    ],
  },
  intro_video_plan: {
