...
状态：`Accepted / V1-1 to V1-7 done / V1-8 next`
...
第一项开发任务已经切换为V1-8两用户并发与恢复：验证两个邀请账号在不同项目并行运行时的数据、强度、任务、费用事件、Provider taskId和产物隔离，并验证同项目单写者与恢复不重复提交。V1-7已完成专项150/150、全量、生产构建和closeout。

V1-8期间仍不得调用真实图片/视频Provider、制作新整包、移动既有标签，也不得由外部Codex替Main Agent选择创意、批准课程锚点或决定返修范围。
