...
从 M54 起，长期能力建设分为三条能力子主线，但都归属于同一 Git 主线：

| 子主线 | 路线文档 | 目标 |
| --- | --- | --- |
| 前端聊天式工作台 | `docs\ui\frontend-workbench\local-real-mvp-m54a-frontend-workbench-roadmap.md` | 输入、消息、附件、反馈、侧栏、产物轨体验升级 |
| 后端对话智能体 | `docs\stages\local-real-mvp-m54b-agentic-conversation-architecture-plan.md` + `docs\stages\local-real-mvp-m54b-agentic-conversation-implementation-roadmap.md` | 主 Agent、能力注册表、工具调度、PromptPack、评测与 checkpoint |
| 智能体交付快路径 | `docs\stages\local-real-mvp-m55-agentic-delivery-fast-path-plan.md` | 一句话需求、多步工具计划、PPT/图片/视频/交付包并发接入与前端展示 |

并行开发规则：

- 三条能力子主线可以拆任务并行，但共享 `ConversationDecisionV2` 合同。
