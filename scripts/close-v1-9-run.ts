...
const CLOSEOUT_LOCK_FILE_NAME = ".external-acceptance-closeout.lock";
const CLOSEOUT_LOCK_STALE_MS = 30 * 60 * 1_000;
...
type CloseoutDependencies = {
  ...
  renameActivePointer?: (from: string, to: string) => Promise<void>;
...
export async function closeV1_9RunAfterExternalAcceptance(input: {
  ...
  const lockPath = path.join(runRoot, CLOSEOUT_LOCK_FILE_NAME);
  const createRandomBytes = input.dependencies?.randomBytes ?? randomBytes;
  const lockToken = createRandomBytes(12).toString("hex");
  await acquireCloseoutLock(lockPath, lockToken);
  ...
  try {
    ...
    await writeImmutableFile(reportPath, reportBytes, "v1_9_external_acceptance_report_already_exists");
    ...
    await assertBytesUnchanged(context.manifestPath, context.manifestBytes, "v1_9_run_manifest_mutated");
    await assertBytesUnchanged(context.pointerSourcePath, context.pointerBytes, "v1_9_active_run_pointer_mutated");
    if (nextState !== context.state) {
      await assertBytesUnchanged(context.statePath, context.stateBytes, "v1_9_run_state_mutated_during_closeout");
      await writeJsonAtomic(context.statePath, nextState, createRandomBytes);
    }
    await assertCommittedRound(context.statePath, roundCommit);
    ...
    await assertBytesUnchanged(context.manifestPath, context.manifestBytes, "v1_9_run_manifest_mutated");
    const renameActivePointer = input.dependencies?.renameActivePointer ?? rename;
    if (context.activePointerPresent) {
      if (await fileExists(context.closedPointerPath)) throw new Error("v1_9_closed_pointer_already_exists");
      await renameActivePointer(context.activePointerPath, context.closedPointerPath);
    }
    const closedPointerBytes = await readFile(context.closedPointerPath).catch(() => {
    ...
    await releaseCloseoutLock(lockPath, lockToken);
  }
}
...
async function resolveCloseoutContext(rootDir: string, expectedRunId: string): Promise<CloseoutContext> {
  ...
  const activePointerPresent = await fileExists(activePointerPath);
  const pointerSourcePath = activePointerPresent ? activePointerPath : closedPointerPath;
  const pointerBytes = await readFile(pointerSourcePath).catch(() => {
  ...
  const [manifestBytes, stateBytes] = await Promise.all([readFile(manifestPath), readFile(statePath)]);
  ...
  return {
    ...
    lockPath: path.join(runRoot, CLOSEOUT_LOCK_FILE_NAME),
  };
}
...
async function assertHistoricalRepairHandoff(
  ...
  const handoffBytes = await readFile(resolveRunRelativeFile(context.runRoot, round.repairHandoffPath)).catch(() => {
...
async function assertPackageFileBinding(context: CloseoutContext, report: V1_9ExternalAcceptanceReport) {
  ...
  if (await sha256File(packagePath) !== report.packageSha256) {
...
async function assertCommittedRound(statePath: string, expected: ReturnType<typeof createRoundCommit>) {
  const state = normalizeV1_9RunState(parseJson(await readFile(statePath), "v1_9_run_state_invalid"));
...
async function acquireCloseoutLock(lockPath: string, token: string) {
  await mkdir(path.dirname(lockPath), { recursive: true });
  const bytes = serializeJson({ pid: process.pid, token, createdAt: new Date().toISOString() });
  for (let attempt = 0; attempt < 4; attempt += 1) {
    try {
      await writeFile(lockPath, bytes, { flag: "wx" });
      return;
    } catch (error) {
      if (!isNodeError(error) || error.code !== "EEXIST") throw error;
      const existingBytes = await readFile(lockPath).catch((readError) => {
        if (isNodeError(readError) && readError.code === "ENOENT") return null;
        throw readError;
      });
      if (!existingBytes) continue;
      const existingStat = await stat(lockPath).catch((statError) => {
        if (isNodeError(statError) && statError.code === "ENOENT") return null;
        throw statError;
      });
      if (!existingStat) continue;
      const lock = inspectExistingCloseoutLock(existingBytes, existingStat.mtimeMs);
      if (!lock.canTakeOver) {
        throw new Error("v1_9_external_acceptance_closeout_locked");
      }
      const confirmedBytes = await readFile(lockPath).catch((readError) => {
        if (isNodeError(readError) && readError.code === "ENOENT") return null;
        throw readError;
      });
      if (!confirmedBytes || !confirmedBytes.equals(existingBytes)) continue;
...
function inspectExistingCloseoutLock(bytes: Buffer, mtimeMs: number) {
  const malformedIsStale = Date.now() - mtimeMs > CLOSEOUT_LOCK_STALE_MS;
  let record: Record<string, unknown>;
  try {
    record = requiredRecord(
      parseJson(bytes, "v1_9_external_acceptance_lock_invalid"),
      "v1_9_external_acceptance_lock_invalid",
    );
    assertOnlyFields(record, ["pid", "token", "createdAt"], "v1_9_external_acceptance_lock_invalid");
  } catch {
    if (malformedIsStale) return { canTakeOver: true };
    throw new Error("v1_9_external_acceptance_lock_invalid");
  }
  const pid = Number(record.pid);
  const createdAt = Date.parse(requiredText(record.createdAt, "v1_9_external_acceptance_lock_invalid"));
  requiredText(record.token, "v1_9_external_acceptance_lock_invalid");
  if (!Number.isSafeInteger(pid) || pid <= 0 || !Number.isFinite(createdAt)) {
    if (malformedIsStale) return { canTakeOver: true };
    throw new Error("v1_9_external_acceptance_lock_invalid");
  }
  const lastOwnerEvidenceAt = Math.max(createdAt, mtimeMs);
  const ownerEvidenceIsStale = Date.now() - lastOwnerEvidenceAt > CLOSEOUT_LOCK_STALE_MS;
  return { canTakeOver: ownerEvidenceIsStale || !processIsAlive(pid) };
}

async function releaseCloseoutLock(lockPath: string, token: string) {
  const value = await readFile(lockPath).catch(() => null);
  if (!value) return;
  const record = requiredRecord(parseJson(value, "v1_9_external_acceptance_lock_invalid"), "v1_9_external_acceptance_lock_invalid");
  if (record.token !== token) throw new Error("v1_9_external_acceptance_lock_ownership_lost");
  await rm(lockPath, { force: true });
}
...
async function writeImmutableFile(filePath: string, bytes: Buffer, conflictCode: string) {
  try {
...
async function writeJsonAtomic(filePath: string, value: unknown, createRandomBytes: (size: number) => Buffer) {
  const temporaryPath = `${filePath}.tmp-${process.pid}-${createRandomBytes(6).toString("hex")}`;
  try {
    await writeFile(temporaryPath, serializeJson(value), { flag: "wx" });
...
async function assertBytesUnchanged(filePath: string, expected: Buffer, errorCode: string) {
  if (!(await readFile(filePath)).equals(expected)) throw new Error(errorCode);
}
