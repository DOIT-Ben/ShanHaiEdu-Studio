        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <Badge tone="bronze">演示工作台</Badge>
              <Badge tone="info">当前：导入视频策划卡</Badge>
            </div>
            <h1 className="title-lg mt-2">认识三角形公开课</h1>
          ...
          <div className="rounded-lg border bg-card px-3 py-2 text-right">
            ...
            <div className="mt-1 text-lg font-semibold text-bronze">3 / 7</div>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-7 gap-2 max-lg:grid-cols-4 max-sm:grid-cols-2">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={step.key}
                className={cn(
                  "flex min-h-11 items-center gap-2 rounded-lg border px-2.5 py-2 text-xs",
                  done && "border-success/20 bg-success/5 text-success",
                  active && "border-bronze/30 bg-bronze/8 text-bronze",
        ...
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-4 p-5">
          ...
          {messages.map((message) => (
            <article
              key={message.id}
              className={cn(
                "max-w-[86%] rounded-xl border p-4 shadow-sm",
                message.speaker === "teacher" ? "ml-auto bg-primary text-primary-foreground" : "mr-auto bg-card",
                message.tone === "focus" && "border-bronze/35 bg-bronze/5",
                message.tone === "warning" && "border-warning/25 bg-warning/5",
              )}
            >
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {["城市里的三角形力量", "纸条挑战实验", "屋顶为什么不倒"].map((title, index) => (
                <div
                  key={title}
                  className={cn(
                    "rounded-lg border p-3 transition duration-150 hover:-translate-y-0.5 hover:border-bronze/40 hover:shadow-sm",
