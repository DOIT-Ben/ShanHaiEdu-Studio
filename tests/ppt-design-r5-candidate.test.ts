...
describe("R5 compact PPT design candidate contract", () => {
  ...
  it("asks the model for the compact R5 candidate instead of the production package", () => {
    ...
    expect(serialized).toContain("ppt-design-candidate.v1");
  ...
  it("requires a real model candidate, computes its digest, and exposes a structurally usable package", async () => {
    const client = {
      responses: {
        create: async () => ({
          output_text: JSON.stringify({
            ...
            artifactDraft: {
              ...
              structuredContentJson: JSON.stringify({ pptDesignCandidate: candidateInput() }),
            },
  ...
  it("refuses to promote a candidate bound to a different TaskBrief digest", async () => {
    const projection = createPptDesignCandidateProjection(candidateInput());
    ...
    expect(result).toMatchObject({ status: "failed", errorCategory: "validation" });
  });
  ...
  it("promotes a candidate-only model result without requiring a production PptDesignPackage", async () => {
    ...
    const runtime: AgentRuntime = {
      async run(input) {
        return {
          status: "succeeded",
          run: { runId: input.runId, projectId: input.projectId, task: input.task, runtimeKind: "openai", status: "succeeded" },
          assistantMessage: { title: "逐页设计候选已形成", body: "已形成可供生产设计扩展使用的候选。" },
          artifactDraft: {
            nodeKey: "ppt_design",
            kind: "ppt_design",
            title: "百分数逐页设计候选",
            ...
            generationMode: "model_generated",
            isReadyForTeacherReview: true,
            structuredContent: { pptDesignCandidate: projection.candidate },
          },
          nextSuggestedAction: { type: "review_artifact", label: "继续完善课件" },
        };
      },
    ...
    const result = await runCapabilityWithAgentRuntime({
      ...
      projectContext: { grade: "五年级", subject: "数学", topic: "百分数", requestedOutputs: ["ppt"] },
      approvedArtifacts: runtimeInput().approvedArtifacts,
    });

    expect(result).toMatchObject({
      status: "succeeded",
      artifactDraft: { structuredContent: { pptDesignCandidate: expect.any(Object) } },
    });
