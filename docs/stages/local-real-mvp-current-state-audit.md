...
- PPTX 已完成图片、动画和视觉精修。
- 视频下载 route、材料包视频资产、在线播放或教师 UI 真实视频生成入口已完成。
- 已具备账号、权限或生产级多人协作。
- 已完成生产部署或公网发布。

当前成熟度判断：

- 内部骨架成熟度：约 91%-95%。核心 workflow、后端持久化、浏览器主链路、产物复用输入、窄屏/Firefox 覆盖、Markdown 下载交付、PPTX 最小下载、最终交付口径同步、ZIP 材料包下载、真实 OpenAI-compatible smoke、PPT 固定样本、真实 Coze PPT smoke、Coze PPT artifact adapter、图片真实 API smoke、图片 artifact adapter、视频真实 API smoke、视频 artifact adapter、阶段测试与文档闭环已经成形。
- 生产就绪度：约 57%-65%。真实文本 smoke、Coze PPT `/run` smoke、Coze PPT artifact adapter、图片 live smoke、图片 artifact adapter、视频 live smoke 与视频 artifact adapter 已通过，但业务节点真实模型全面接入、Coze 官方 OpenAPI 主链路、图片下载/材料包集成、视频下载/材料包集成、账号权限、生产部署、安全与运维仍未完成。
...
- M20 视频 live smoke 已通过，M21 已接入后端 artifact adapter；但当前尚未提供视频下载 route、最终材料包视频资产、教师 UI 入口、生产队列或质量验收。
...
1. 进入 M22：视频下载 route 或材料包视频资产集成，按 `.tmp` 读取约束、脱敏响应和教师可见边界推进。
