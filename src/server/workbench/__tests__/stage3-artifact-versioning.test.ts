...
import { GET as getApprovedInputsRoute } from "@/app/api/workbench/projects/[projectId]/approved-inputs/route";
...
describe("Backend Workflow Lite Stage 3 artifact versioning", () => {
  ...
  it("keeps downstream inputs on the approved old version until the new version is approved", async () => {
    const service = createWorkbenchService();
    const project = await service.createProject({ title: "Stage 3 输入保持项目" });
    const v1 = await service.saveArtifact(project.id, {
      nodeKey: "requirement_spec",
      kind: "requirement_spec",
      title: "需求规格",
    ...
    expect(inputs.map((artifact) => artifact.summary)).toEqual(["旧输入"]);
  });
  ...
  it("switches the approved pointer when approving a regenerated version", async () => {
    ...
    expect(node?.approvedArtifactId).toBe(v2.id);
    expect(inputs.map((artifact) => artifact.summary)).toEqual(["新确认"]);
  });
  ...
  it("keeps approved-inputs route on the old version after route regeneration", async () => {
    const service = createWorkbenchService();
    const project = await service.createProject({ title: "Stage 3 route 输入保持项目" });
    const v1 = await service.saveArtifact(project.id, {
      nodeKey: "requirement_spec",
      kind: "requirement_spec",
      title: "需求规格",
      status: "needs_review",
      summary: "route old input",
      markdownContent: "# route old",
    });
    ...
    const regenerateResponse = await postRegenerateRoute(
      new Request("http://localhost", {
        ...
        body: JSON.stringify({
          title: "需求规格重做",
          status: "approved",
          summary: "route new input",
          markdownContent: "# route new",
        }),
      }),
      { params: Promise.resolve({ projectId: project.id, artifactId: v1.id }) },
    );
    const regenerateBody = await regenerateResponse.json();
    const inputsResponse = await getApprovedInputsRoute(
    ...
    expect(regenerateBody.artifact).toMatchObject({ status: "needs_review", isApproved: false });
    expect(inputsResponse.status).toBe(200);
    expect(inputsBody.artifacts.map((artifact: { summary: string }) => artifact.summary)).toEqual(["route old input"]);
  });
