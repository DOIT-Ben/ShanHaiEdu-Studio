...
状态：`Accepted / V1-1 to V1-8 done / V1-9 next`
...
| V1-5 | 生成强度 | 四档滑杆、默认标准、升级建议、积分趋势和确认已经封板 | `done`：不暴露模型、不静默升级、极致需要二次确认 |
| V1-6 | PPT内部编排闭环 | Critic审查持久化、HumanGate批准边界、结构化页级返修和Main Agent编排已经封板 | `done`：专项71/71；决策来自产品Agent/Tool，未调用真实Provider |
| V1-7 | 视频内部编排闭环 | 课程锚点六硬门、成片九项审查、双HumanGate、实际证据门和结构化镜头返修输入已经封板 | `done`：专项150/150；未调用真实媒体Provider |
| V1-8 | 两用户并发 | 双password actor、双项目、强度/租约/job/taskId/shot/预算/产物隔离和恢复已经封板 | `done for single-process V1 topology`；多client写限制转V1-10拓扑门 |
...
第一项开发任务已经切换为V1-9产品内真实E2E：先关闭FFmpeg/ffprobe、五类成片证据和最终包版本一致性缺口，再由产品Main Agent从界面独立运行一次真实任务；外部Codex只做成包后黑盒审核。V1-8已完成单进程两用户并发封板。

V1-9只允许一次受控真实E2E及必要的定点复验；不得移动既有标签，也不得由外部Codex替Main Agent选择创意、批准课程锚点或决定返修范围。
