...
先完成上线门槛反馈中心和 M68 对话控制，再继续 M54-A 其余第一档收口：

1. 完成 `docs\stages\local-real-mvp-m67-beta-feedback-center-plan.md`：真实保存、分类提示、截图粘贴和受控查看。
2. 完成 M68 普通聊天/业务任务分流、自然语言确认改道和对话承诺/执行一致性。
3. 收口首次欢迎态、头像菜单和全局反馈入口。
4. 完成附件拖入、图片粘贴、PDF/DOCX/图片真实状态。
5. 完成模型/工具菜单并清理未接假入口。
