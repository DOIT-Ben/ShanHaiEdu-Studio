...
import { AdminUserManagementError, resetManagedUserPassword } from "@/server/auth/admin-user-management";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const result = await resetManagedUserPassword({ userId, newPassword: body.newPassword, actorUserId: guard.actorUserId });
...
async function requireAdminWithCsrf(request: Request): Promise<{ actorUserId: string; response?: undefined } | { actorUserId?: undefined; response: Response }> {
  ...
  if (!canManageUsers(session.actor) || !session.publicSession) return { response: NextResponse.json({ error: "无权执行此操作。" }, { status: 403 }) };
