...
当前唯一动作是按活动计划实施VR-A14/A16：Next启动后从精确SQLite身份派生typed recovery disposition，runner只传冻结路径和可选证据locator，不再传恢复布尔或恢复类型；project、task、epoch、teacher message、TurnJob、checkpoint、actor/auth/session、plan任一缺失或错绑均在服务ready前失败关闭，不取项目全局latest。真实Provider连续3组仍等待用户另行批准。
