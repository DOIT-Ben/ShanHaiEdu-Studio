...
本审计用于收口 `mainline/local-real-mvp` 在 M0-M31 后的真实状态，明确哪些能力已经通过本地验证、哪些只是 readiness、哪些仍不能宣称完成。
...
- 生产就绪度：约 82%-86%。真实文本 smoke、Coze PPT `/run` smoke、Coze PPT artifact adapter、图片 live smoke、图片 artifact adapter、图片下载 route、图片材料包集成、视频 live smoke、视频 artifact adapter、视频下载 route、视频材料包集成、教师 UI 真实生成入口、浏览器联动验证、可配置部署卷准备、本地权限最小闭环、真实生成任务状态基础、Next standalone 准备、生产预检和本地生产 SQLite 初始化已通过，但业务节点真实模型全面接入、Coze 官方 OpenAPI 主链路、独立 worker、公网认证、远端部署、安全与运维仍未完成。
