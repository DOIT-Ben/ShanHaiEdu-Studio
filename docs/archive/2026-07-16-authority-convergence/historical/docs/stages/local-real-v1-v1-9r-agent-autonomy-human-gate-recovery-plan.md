...
状态：`Accepted / R0-R5 complete / V1-9 unique real E2E next / V1-10 public cutover still blocked`
...
- R5的PPT模型合同只承载目标、Evidence主张、叙事、教学动作和逐页视觉语义。`TaskBrief.digest`、Artifact ID/version/digest与candidate digest由服务端基于已验证`ExecutionEnvelope`、有效TaskBrief和当前可信`ppt_draft`投影；模型回传的同名权威字段一律不采信。
- candidate-only `ppt_design_draft`通过最低候选Validator即可持久化；缺少完整`PptDesignPackage`仍禁止任何真实图片、PPTX、样张或整包Provider调用。若结果携带生产包，则继续执行完整生产Validator，不降低V1-9门禁。
