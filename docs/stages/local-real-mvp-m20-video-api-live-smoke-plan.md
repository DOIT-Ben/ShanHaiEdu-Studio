...
- 视频 provider 可能长时间排队；脚本必须支持复查已有任务并把持续排队分类为 stuck，不能靠同步长等无限消耗。
...
- 支持 `VIDEO_SMOKE_TASK_ID` 和 `.tmp\video-smoke\last-task.json` 复查已有任务；该缓存文件只留在 ignored `.tmp`，不提交。
- 失败时输出脱敏任务摘要，只记录状态、进度和是否存在结果 URL，不记录任务 id 或远程 URL。

必要自研：

- 增加视频 submit/query 响应解析测试。
- 增加可恢复查询、脱敏状态摘要和 stuck 分类测试。
