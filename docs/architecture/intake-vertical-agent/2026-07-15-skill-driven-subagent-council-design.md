...
第一版只实现 B 的最小形态，不直接实现并发和 Council：

```text
一个 ShanHai ParentRun
-> 一个叶子 DelegatedRun
-> 一个 Codex Child Loop
-> ParentRun 在依赖屏障耐久等待
-> ChildResultEnvelope 持久化
-> ParentRun 恢复并作出 AcceptanceDecision
```

父任务“等待”不是占住进程直到子任务结束，而是进入 `awaiting_delegated_result`，释放 Worker、模型线程和数据库事务，由完成事件耐久恢复。每个 ParentRun 同时最多一个活动 DelegatedRun；子智能体不能创建孙智能体；父子不能同时推进同一业务任务。

完整联合边界以 `2026-07-15-hermes-vertical-joint-integration-contract.md` 和 Hermes H07 为准。
...
Skill 声明的 HumanGate 只是候选协作位置。它不能取消政策、风险、费用或授权规则要求的 Gate，不能自行扩大权限、预算、时限和资源作用域，也不能把任何候选结果直接批准为业务完成。
...
子智能体只返回持久化后的 `ChildResultEnvelope`：

```typescript
interface ChildResultEnvelope {
  rootRunId: string;
  parentRunId: string;
  delegatedRunId: string;
  attemptId: string;
  ...
  usage: RuntimeUsage;
  needsInput?: StructuredQuestion;
  failureClass?: string;
  nonAuthoritativeRecommendation?: string;
}
```

子智能体不能直接更新 Project、IntentEpoch、HumanGate、QualityDecision 或 Artifact Promotion。它不能直接向教师提问；`needs_input` 返回父任务，由父任务结合业务策略决定是否创建 HumanGate。

`completed_candidate` 只表示执行合同所要求的结构化结果已经持久化，不表示 Validator 已通过或业务已接受。父任务恢复后必须重新读取权威状态并产生 `AcceptanceDecision`：`accepted`、`repair_requested`、`rejected` 或 `stale`。
...
`needs_input` 是 ChildResultEnvelope 的结构化阻断原因，不让子任务直接拥有面向教师的 `waiting_human`。只有 ParentRun 能根据该结果进入 ShanHai 的 HumanGate 生命周期。

必须持久化：

- rootRunId、parentRunId、delegatedRunId、taskId、agentProfile、runtimeKind；
- RuntimeThreadBinding 和 CodexTurn 引用；
- contextSnapshotId、capabilityGrantId、resourceScopes；
...
2. 第一阶段每个 ParentRun 同时最多一个活动 DelegatedRun，不启用 fan-out。
3. 主智能体是唯一教师对话入口；子Agent需要信息时返回 `needs_input`。
4. 子Agent默认不继承完整主对话，只读取冻结的 ContextSnapshot/ContextPackage。
5. 每个任务必须有超时、心跳、取消、幂等键和预算。
6. Tool 权限按交集计算，不做隐式继承。
7. 每个分支独立记录 Token、Provider 成本和积分。
8. Council 必须有最大角色数、最大阶段数和最大修订轮数。
9. 生成 Agent 不能担任最终业务 Validator。
10. 失败文本必须分类并安全摘要，不能把内部错误直接展示给教师。
11. Codex Thread 不是业务状态源；DelegatedRun 与 Attempt 必须由 ShanHai 持久化。
12. 完成的含义是 `completed_candidate` 已持久化，业务完成仍需父任务验收。
...
### VA03-1：父等待的单一受限 Worker

只支持一个边界明确、无付费媒体副作用的叶子子智能体。每个 ParentRun 同时最多一个活动 DelegatedRun；父任务以 `awaiting_delegated_result` 耐久等待，结果以 `completed_candidate` 返回。先用 Fake Child Adapter 和 Native 对照路径验证生命周期、恢复与验收，再绑定 Codex App Server。

### VA03-1R：独立 Reviewer

在不开放并发的前提下，验证只读 Reviewer 和生成 Worker 的职责隔离。Reviewer 只提交 ValidationReport，不批准 QualityDecision。
...
- 父子任务谱系完整率 100%；
- 父任务等待期间占用 Worker、模型线程或数据库事务数量 0；
- `completed_candidate` 未经 AcceptanceDecision 进入业务完成数量 0；
