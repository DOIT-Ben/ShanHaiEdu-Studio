...
状态：`V1-3 closed / ready to plan V1-4 HumanGate and interruption`

## 1. 交接结论

V1-3已经完成专项、全量、构建、SQLite和人工diff审查并形成closeout。下一对话直接进入V1-4计划与测试定义，再实现HumanGate与自然语言确认、拒绝、暂停、取消、改道、改大纲和局部返修；不得重新执行V1-1至V1-3，也不得把共享ReAct运行时提前描述为PPT或视频领域闭环。
...
- 尚未实现：V1-4至V1-10各阶段；共享运行时不替代V1-6/V1-7领域闭环。
...
8. `docs\stages\local-real-v1-v1-3-main-agent-controlled-react-closeout.md`
9. V1-4计划与测试计划（创建后）及相邻HumanGate/IntentEpoch代码和测试
...
```text
完成V1-4 HumanGate与自然语言打断：
先形成阶段计划与测试计划，
再闭环确认、拒绝、暂停、取消、改道、改大纲和局部返修，
保证旧action、旧IntentEpoch与迟到结果不能污染新分支。
```

执行顺序：

1. 读取V1-3 closeout和本交接，不回退已经封板的ReAct、合同与测试。
2. 先写V1-4阶段计划与测试计划，冻结自然语言意图分类、歧义处理、影响分析与恢复边界。
3. 覆盖确认、拒绝、暂停、取消、改道、改大纲和局部返修，不把快捷按钮当作唯一入口。
4. 实质修改必须推进IntentEpoch并使旧actionId失效；迟到Tool结果不得提升到新分支。
5. 能唯一理解时直接执行安全控制；多个候选分支无法唯一判断时只问一个具体消歧问题。
6. 保留历史版本和失败审计，只失效受影响下游，不无差别重跑整条链。
7. V1-4专项和全量门禁关闭前，不进入四档强度或PPT/视频真实Provider验证。

### 7.1 V1-4已冻结的起步边界

V1-4不得只增加几个确认关键词后宣称自然语言打断完成。进入V1-4时至少要满足：

1. 自然语言控制与快捷按钮共用同一HumanGate/action合同，不能产生两套授权语义。
2. “继续”结合唯一活动计划解析；有歧义时消歧，不能误报为无有效确认。
3. 暂停、取消和改道必须持久化，并让旧计划退出活动状态。
4. 修改大纲或局部页/镜头时先做影响分析，只使受影响下游stale并保留可复用Artifact。
5. IntentEpoch、租约与结果提升共同阻止旧分支迟到结果污染新分支。
...
- V1-4开发期间不调用真实媒体Provider，不制作新验收包。
