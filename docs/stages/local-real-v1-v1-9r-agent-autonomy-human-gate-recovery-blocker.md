...
状态：`repository control plane passed / R5 blocked by tool-bearing Main Agent Responses plus structured text runtime health / desktop real-agent evidence pending / 390px deferred until after V1 / does not authorize V1-9 real Provider E2E`
...
> 当前收敛结论：仓内控制面通过。primary能完成Main Agent多轮控制请求但结构化业务文本Runtime仍可180秒timeout；fallback最小Responses曾成功，但真实带Tool Main Agent请求并发返回403。R5仅待同一通道完整完成这两类请求后的真实桌面证据；390px不属于V1发布前门禁。没有新的通道级健康变化前不再做等价请求。
