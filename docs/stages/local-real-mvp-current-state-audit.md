...
本审计用于收口 `mainline/local-real-mvp` 在 M0-M39 后的真实状态，明确哪些能力已经通过本地验证、哪些只是 readiness、哪些仍不能宣称完成。
